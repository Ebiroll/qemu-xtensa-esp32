...
static void esp32_sha_text_reg_byteswap_result(Esp32ShaState* s, uint32_t* dst, size_t len_words)
{
    for (int i = 0; i < len_words; ++i) {
...
static void esp32_sha_finish(Esp32ShaState *s, QCryptoHashAlgorithm hash_alg)
{
    ...
    if (s->full_text_len % hash_block_bytes[hash_alg] != 0) {
        ...
        if (byte_count > s->full_text_len) {
            error_report("esp32_sha_finish: invalid byte count %" PRIx32 ",%" PRIx32 "\n", byte_count, s->full_text_len);
            qcrypto_hash_bytes(hash_alg, (const char*) s->full_text, byte_count, &result, &result_len, &error_abort);
            esp32_sha_text_reg_byteswap_result(s, (uint32_t*) s->hash, result_len/4);
        } else {
            qcrypto_hash_bytes(hash_alg, (const char*) s->full_text, byte_count, &result, &result_len, &error_abort);
            error_report("esp32_sha_len:%ld",result_len);
            esp32_sha_text_reg_byteswap_result(s, (uint32_t*) s->hash, result_len/4);
        ...
        case 0x0040 ... 0x007C:
            {
                //printf("Read hash \n"); 
                if (addr==0x0040)  {
                    esp32_sha_finish(s, index_to_alg(s->mode));                             
                }  
                       ...
                       uint64_t value, unsigned int size)
{
    ...
    switch (addr) {
        ...
        case 0x0010: // SHA_START_REG
            esp32_sha_update_text(s, index_to_alg(s->mode));
        break;
