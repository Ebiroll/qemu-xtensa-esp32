...
static void nvic_writel(nvic_state *s, uint32_t offset, uint32_t value)
{
    ...
    case 0xd28: /* Configurable Fault Status.  */
        ...
        qemu_log_mask(LOG_UNIMP,"writel:cfsr now: %08X\n", cpu->env.v7m.cfsr);
        break;
    case 0xd2c: /* Hard Fault Status.  */
        cpu = ARM_CPU(current_cpu);
        cpu->env.v7m.hfsr &= ~value;
        qemu_log_mask(LOG_UNIMP,"writel:hfsr now: %08X\n", cpu->env.v7m.hfsr);
        break;
    case 0xd30: /* Debug Fault Status.  */
        cpu = ARM_CPU(current_cpu);
        cpu->env.v7m.dfsr &= ~value;
        qemu_log_mask(LOG_UNIMP,"writel:dfsr now: %08X\n", cpu->env.v7m.dfsr);
        break;
    case 0xd34: /* Mem Manage Address.  */
        cpu = ARM_CPU(current_cpu);
        cpu->env.v7m.mmfar = value;
        qemu_log_mask(LOG_UNIMP,"writel:mmfar now: %08X\n", cpu->env.v7m.mmfar);
        break;
    case 0xd38: /* Bus Fault Address.  */
        cpu = ARM_CPU(current_cpu);
        cpu->env.v7m.bfar = value;
        qemu_log_mask(LOG_UNIMP,"writel:bfar now: %08X\n", cpu->env.v7m.bfar);
        break;
    case 0xd3c: /* Aux Fault Status.  */
    ...
    case 0xd94: /* MPU control register.  */
        cpu = ARM_CPU(current_cpu);
        if (cpu->pmsav7_dregion == 0) {
            qemu_log_mask(LOG_UNIMP,"writel:mpu_ctrl -- no regions!\n");
            break;
        }
    ...
    case 0xd98: /* MPU_RNR.  */
        ...
        qemu_log_mask(LOG_UNIMP,"writel:mpu_rnr, region now: %u\n", cpu->env.cp15.c6_rgnr);
        break;
    case 0xd9c: /* MPU_RBAR: MPU region base address register.  */
    case 0xda4: /* MPU_RBAR_A1.  */
    case 0xdac: /* MPU_RBAR_A2.  */
    case 0xdb4: /* MPU_RBAR_A3.  */
        cpu = ARM_CPU(current_cpu);
        if (cpu->pmsav7_dregion == 0) {
            qemu_log_mask(LOG_UNIMP,"writel:mpu_rbar (%02X) -- no regions!\n", offset);
            break;
        }
        if (value & 0x10) {
            /* region update */
