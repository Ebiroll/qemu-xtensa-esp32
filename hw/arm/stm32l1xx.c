 ...
 *
 * Copyright (C) 2018 Olof Astrand
 *
 * Implementation based on RM0038 Reference manual"
 *
...
static const char *stm32l1xx_periph_name_arr[] = {
            ...
            ARMCPU **cpu)
{
    ...
    DeviceState *rcc_dev = qdev_create(NULL, "stm32l1xx_rcc");
    ...
    DeviceState **gpio_dev = (DeviceState **)g_malloc0(sizeof(DeviceState *) * STM32L1XX_GPIO_COUNT);
    for(i = 0; i < STM32L1XX_GPIO_COUNT; i++) {
        stm32_periph_t periph = STM32_GPIOA + i;
        gpio_dev[i] = qdev_create(NULL, "stm32f2xx_gpio");
        gpio_dev[i]->id = stm32l1xx_periph_name_arr[periph];
...
//        qdev_prop_set_ptr(uart_dev, "stm32_check_tx_pin_callback", (void *)stm32_afio_uart_check_tx_pin_callback);
    ...
    for (i = 0; i < ARRAY_LENGTH(spi_desc); ++i) {
        ...
        stm->spi_dev[i]->id = stm32l1xx_periph_name_arr[periph];
...
#define dummy_dev(name, start, size) do {\
    ...
    i2c1->id = stm32l1xx_periph_name_arr[STM32_I2C1];
    ...
    i2c2->id = stm32l1xx_periph_name_arr[STM32_I2C2];
